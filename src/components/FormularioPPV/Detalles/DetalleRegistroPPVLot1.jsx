import {
  ...
  calcularHorasIdeal
} from '../../../components/FormularioPPV/helpers/calculos'

const DetalleRegistroPPVLot1 = ({ registro: r }) => {
  ...
  const retrasosASA = r.retrasosASA || r.retrasos_asa
  const retrasosAlmacen = r.retrasosAlmacen || r.retrasos_almacen
  ...
  const producidoIdeal = calcularEnvasadoIdeal(r.horasTrabajadas ?? r.horas_trabajadas, r.cantidadIdealPorHora ?? r.cantidad_ideal_por_hora)
  const horasIdeal = r.horasIdeales ?? r.horas_ideales ?? calcularHorasIdeal(r.cantidadProducida ?? r.cantidad_producida, r.cantidadIdealPorHora ?? r.cantidad_ideal_por_hora)
    ...
    <div className="card-body p-4">
      ...
      <div className="card mb-4 shadow-sm">
        <div className="card-header bg-warning text-dark">
          <h5 className="mb-0">Información General y Tiempos (PPV Lot1)</h5>
        </div>
        <div className="card-body">
          <div className="row">
            ...
            <div className="col-md-6 mb-3">
              <div className="card h-100">
                <div className="card-body">
                  ...
                  <div className="text-center small text-muted">
                    Cantidad Producida / Cantidad Programada
                  </div>
                  <div className="text-center fw-bold">
                    {formatearCantidad(r.cantidadProducida ?? r.cantidad_producida)} / {formatearCantidad(r.cantidadProgramadaDiaria ?? r.cantidad_programada_diaria)}
                  </div>
                </div>
              </div>
            </div>
                ...
                <table className="table table-sm table-bordered mb-0">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>Eficacia</td>
                      <td className="small">
                        (Cantidad Producida / Cantidad Programada) × 100 <br />
                        <span className="text-muted">({formatearCantidad(r.cantidadProducida ?? r.cantidad_producida)} / {formatearCantidad(r.cantidadProgramadaDiaria ?? r.cantidad_programada_diaria)})</span>
                      </td>
                    ...
                    <tr>
                      <td>Cantidad Producida Teórica</td>
                      <td className="small">
                      ...
                      <td>{formatearCantidad(r.cantidadProducidaTeorica ?? r.cantidad_producida_teorica)}</td>
                    </tr>
                    <tr>
        ...
        <div className="card-body">
          <table className="table table-sm table-bordered mb-0">
            <tbody>
              ...
              <tr>
                <td>Producido Ideal (según horas trabajadas)</td>
                <td>{formatearCantidad(producidoIdeal)}</td>
              </tr>
              <tr>
                <td>Horas Ideal (según cantidad producida)</td>
      ...
      <div className="card mb-4 shadow-sm">
        ...
        <div className="card-body">
          <div className="row g-3">
            <DetallesRetrasos titulo="Factores que NO afectan eficiencia" data={factoresNoEficiencia} color="info" />
            <DetallesRetrasos titulo="Retrasos por Producción" data={retrasosProduccion} color="danger" />
            <DetallesRetrasos titulo="Retrasos por Control de Calidad" data={retrasosCalidadControl} color="warning" />
            <DetallesRetrasos titulo="Retrasos por Mantenimiento" data={retrasosMantenimiento} color="primary" />
            <DetallesRetrasos titulo="Retrasos por ASA" data={retrasosASA} color="secondary" />
            <DetallesRetrasos titulo="Retrasos por Almacén" data={retrasosAlmacen} color="dark" />
            <DetallesRetrasos titulo="Otros Factores" data={otrosFactores} color="secondary" />
          </div>
        </div>
      </div>
...
export default DetalleRegistroPPVLot1
