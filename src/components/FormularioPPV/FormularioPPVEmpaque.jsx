...
// ==== Estado inicial para PPV Empaque ====
const FORMULARIO_PPV_EMPAQUE = {
...
const FormularioPPVEmpaque = () => {
  const navigate = useNavigate()
  const [seccionActiva, setSeccionActiva] = useState('basicos')
  const [showModal, setShowModal] = useState(false)
  const [guardando, setGuardando] = useState(false)

  const [formulario, setFormulario] = useState({ ...FORMULARIO_PPV_EMPAQUE })
  ...
  useEffect(() => {
    ...
    // Horas ideales calculadas para PPV (puede ser igual a bottlepack si usas los mismos conceptos)
  ...
  const guardarRegistro = async () => {
    ...
    try {
      ...
      await guardarRegistroPPV('ppv_empaque', nuevoRegistro)
      setShowModal(true)
    } catch {
  ...
  const limpiarFormulario = () => {
    setFormulario({ ...FORMULARIO_PPV_EMPAQUE })
  }

  const handleModalClose = () => {
    setShowModal(false)
    navigate('/registros/PPV-EMP-001')
  }
    ...
    <div className="container-fluid py-4 bg-light">
      <div className="card shadow-lg border-0">
        <div className="card-header bg-warning text-dark d-flex align-items-center gap-3">
          ...
          <h3 className="mb-0">Control de Producción PPV - Empaque</h3>
        </div>
        <div className="card-body">
          ...
          <div className="d-flex gap-2 mt-4">
            ...
            <button
              ...
              onClick={() => navigate('/registros/PPV-EMP-001')}
              disabled={guardando}
            >
              Ver Registros
            </button>
...
export default FormularioPPVEmpaque
