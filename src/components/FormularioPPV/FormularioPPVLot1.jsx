...
// ==== Estado inicial para PPV Lot1 ====
const FORMULARIO_PPV_LOT1 = {
...
const FormularioPPVLot1 = () => {
  const navigate = useNavigate()
  const [seccionActiva, setSeccionActiva] = useState('basicos')
  const [showModal, setShowModal] = useState(false)
  const [guardando, setGuardando] = useState(false)

  const [formulario, setFormulario] = useState({ ...FORMULARIO_PPV_LOT1 })
  ...
  useEffect(() => {
    ...
    // Horas ideales calculadas para PPV Lot1
  ...
  const guardarRegistro = async () => {
    ...
    try {
      ...
      await guardarRegistroPPV('ppv_lot1', nuevoRegistro) // Cambia la clave a la tabla/colección que uses
      setShowModal(true)
    } catch {
  ...
  const limpiarFormulario = () => {
    setFormulario({ ...FORMULARIO_PPV_LOT1 })
  }

  const handleModalClose = () => {
    setShowModal(false)
    navigate('/registros/PPV-LOT1-001')
  }
    ...
    <div className="container-fluid py-4 bg-light">
      <div className="card shadow-lg border-0">
        <div className="card-header bg-warning text-dark d-flex align-items-center gap-3">
          ...
          <h3 className="mb-0">Control de Producción PPV - Lote 1</h3>
        </div>
        <div className="card-body">
          ...
          <div className="d-flex gap-2 mt-4">
            ...
            <button
              ...
              onClick={() => navigate('/registros/PPV-LOT1-001')}
              disabled={guardando}
            >
              Ver Registros
            </button>
...
export default FormularioPPVLot1
