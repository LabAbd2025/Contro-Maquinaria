...
const FormularioPGVPVCStd = () => {
  ...
  // Guardar registro (ajusta aquí a tu API/Supabase/LocalStorage)
  const guardarRegistro = async () => {
    if (guardando || showModal) return
    setGuardando(true)
    try {
      // await guardarRegistroPGVPVCStd(formulario)
      setShowModal(true)
    } catch {
  ...
  const handleModalClose = () => {
    setShowModal(false)
    navigate('/registros/PGV-PVC-STD-001') // Cambia al path real si es otro
  }
    ...
    <div className="container-fluid py-4 bg-light">
      <div className="card shadow-lg border-0">
        <div className="card-header bg-primary text-white d-flex align-items-center gap-3">
          <h3 className="mb-0">Control de Producción PGV PVC Std</h3>
        </div>
        <div className="card-body">
          ...
          <div className="d-flex gap-2 mt-4">
            ...
            <button
              ...
              onClick={() => navigate('/registros/PGV-PVC-STD-001')}
              disabled={guardando}
            >
              Ver Registros
            </button>
...
export default FormularioPGVPVCStd
