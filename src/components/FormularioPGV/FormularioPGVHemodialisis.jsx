...
const FormularioPGVHemodialisis = () => {
  ...
  // Autocalcula campos dependientes
  useEffect(() => {
  ...
  // Guardar registro (puedes conectar aquí tu API, Supabase o LocalStorage)
  const guardarRegistro = async () => {
    if (guardando || showModal) return
    setGuardando(true)
    try {
      // await guardarRegistroPGVHemodialisis(formulario)
      setShowModal(true)
    } catch {
  ...
  const handleModalClose = () => {
    setShowModal(false)
    navigate('/registros/PGV-HEMO-001') // Ajusta al path real
  }
    ...
    <div className="container-fluid py-4 bg-light">
      <div className="card shadow-lg border-0">
        <div className="card-header bg-primary text-white d-flex align-items-center gap-3">
          <h3 className="mb-0">Control de Producción PGV Hemodiálisis</h3>
        </div>
        <div className="card-body">
          ...
          <div className="d-flex gap-2 mt-4">
            ...
            <button
              ...
              onClick={() => navigate('/registros/PGV-HEMO-001')}
              disabled={guardando}
            >
              Ver Registros
            </button>
...
export default FormularioPGVHemodialisis
